...
`);
    ...
    // oluşturduğumuz divleri html e gönderme 
...
// dizideki herbir elemanın tıklanma olayını izleme
...
function searchCategory(e) {
    ...
    // elemanları kategori değerine göre alma
    const filtredItems = menu.filter((menuItem) => {
        if(menuItem.category === category) return menuItem
    })
    // hepsi çekildiyse tamamını göster değilse filtrelenmiş diziyi
    if (category === 'all') {
        displayMenuItems(menu);
    } else {
